...
public class SequenceDatabase {
	...
	/**
	 * 
	 * Function created by R. K
	 * prints the list of the sequences to a file.
	 * If error occurred returns 0;
	 * @param filename the name of the file that it will be created
	 * @return 1 if everything is ok, otherwise 0.
	 */
	public int writeSequencesToFile(String filename){
		BufferedWriter out = null;
		FileWriter fstream;
		try {
			fstream = new FileWriter(filename + ".txt", false);
			out = new BufferedWriter(fstream);
		    for (Sequence i : sequences) out.write(i.toString());
		} catch (IOException e) {
			return 0;
		} finally{
		    if(out != null) {
